...
RSpec.describe 'OpenAPIParser 3.2 spec validator (integration)' do
  ...
  describe 'Media Type itemSchema/itemEncoding/prefixEncoding (3.2 additions)' do
    it 'warns on the version-mismatched document under :warn' do
      expect_mismatch_warns('streaming_fields_31.yaml', [:streaming_fields_before32, :streaming_fields_before32, :streaming_fields_before32])
    end

    it 'raises SpecViolationError on the version-mismatched document under :raise' do
      expect_mismatch_raises('streaming_fields_31.yaml', [:streaming_fields_before32, :streaming_fields_before32, :streaming_fields_before32])
    end

    it 'stays clean on the correctly-versioned document' do
      expect_clean('streaming_fields_32.yaml')
    end
  end

  describe 'Discriminator defaultMapping (3.2 addition)' do
    it 'warns on the version-mismatched document under :warn' do
      expect_mismatch_warns('default_mapping_31.yaml', [:default_mapping_before32])
    end

    it 'raises SpecViolationError on the version-mismatched document under :raise' do
      expect_mismatch_raises('default_mapping_31.yaml', [:default_mapping_before32])
    end
  ...
  describe 'Parameter in: querystring (3.2 addition)' do
    it 'warns on the version-mismatched document under :warn' do
      expect_mismatch_warns('querystring_31.yaml', [:querystring_before32])
    end

    it 'raises SpecViolationError on the version-mismatched document under :raise' do
      expect_mismatch_raises('querystring_31.yaml', [:querystring_before32])
    end
  ...
  describe 'Response Object summary (3.2 addition)' do
    it 'warns on the version-mismatched document under :warn' do
      expect_mismatch_warns('response_summary_31.yaml', [:response_summary_before32])
    end

    it 'raises SpecViolationError on the version-mismatched document under :raise' do
      expect_mismatch_raises('response_summary_31.yaml', [:response_summary_before32])
    end
